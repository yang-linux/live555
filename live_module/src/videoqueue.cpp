...
#include <algorithm>
#include <string.h>
VideoQueue::VideoQueue(int nSize)
:m_pBuffer(NULL)
,m_nSize(nSize)
,m_nIn(0)
,m_nOut(0)
...
unsigned long VideoQueue::roundup_power_of_two(unsigned long val)
{
    if((val & (val-1)) == 0)
        return val;
...
unsigned int VideoQueue::Put(const unsigned char *buffer, unsigned int len)
{
	unsigned int l;
...
unsigned int VideoQueue::Get(unsigned char *buffer, unsigned int len)
{
	unsigned int l;
